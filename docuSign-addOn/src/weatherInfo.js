...
exports.handler = async (event) => {
  ...
  if (
    ...
    return {
      eventResponse: wrapResponse(
        "<p><strong>OPENWEATHERMAP_API_KEY</strong> is not configured.</p>",
      ),
    };
  }
  ...
  try {
     ...
     */
    var jobResponse = await requestGet({
     ...
     */
    if (jobResponse.httpResponse.statusCode != 200) {
      return {
        eventResponse: wrapResponse(
          "<p>An error occurred while fetching the details for job " +
            strJobUUID +
            ". " +
            JSON.stringify(jobResponse.httpResponse) +
            "</p>",
        ),
      };
    }
     ...
     * lng fields.
     */
    var objJob = JSON.parse(jobResponse.body);
    if (!objJob.geo_is_valid) {
      return {
        eventResponse: wrapResponse(
          "<p>The weather cannot be determined because there is no location information for this job.</p>",
        ),
      };
    }
     ...
     *
     */
    var bookingsResponse = await requestGet({
      url:
        "https://api.servicem8.com/api_1.0/JobActivity.json?" +
        encodeURIComponent("$filter") +
        "=" +
        encodeURIComponent("job_uuid eq '" + strJobUUID + "'"),
      auth: {
        bearer: event.auth.accessToken, // Use the temporary Access Token that was issued for this event
      },
      headers: {
     ...
     * and we need to end our Lambda function here.
     */
    if (bookingsResponse.httpResponse.statusCode != 200) {
      return {
        eventResponse: wrapResponse(
          "<p>An error occurred while fetching scheduled bookings for Job " +
            strJobUUID +
            ". " +
            JSON.stringify(bookingsResponse.httpResponse) +
            "</p>",
        ),
      };
    }

    var arrBookings = JSON.parse(bookingsResponse.body);
     ...
     *
     */
    var weatherResponse = await requestGet({
      url:
    ...
    if (weatherResponse.httpResponse.statusCode != 200) {
      return {
        eventResponse: wrapResponse(
          "<p>An error occurred while fetching scheduled bookings for Job " +
            strJobUUID +
            ". " +
            JSON.stringify(weatherResponse.httpResponse) +
            "</p>",
        ),
      };
    }
     ...
     */
    var objWeather = JSON.parse(weatherResponse.body);
    ...
    for (let thisBooking of arrBookings) {
      ...
      let intTZDiff =
        ...
        parseInt(thisBooking.start_date.substr(-4, 2)) * 60 +
        parseInt(thisBooking.start_date.substr(-2));
      ...
      let strTime = objBookingTime.toLocaleString("en-US", {
        weekday: "long",
        month: "long",
        day: "numeric",
        hour: "2-digit",
      ...
      for (let thisWeather of objWeather.list) {
        ...
        if (
          intBookingTime >= thisWeather.dt &&
          intBookingTime < thisWeather.dt + 3 * 3600
        ) {
          let strIcon =
            '<img src="http://openweathermap.org/img/w/' +
            thisWeather.weather[0].icon +
            '.png" alt="' +
            thisWeather.weather[0].description +
            '" />';
          strResult +=
            '<p class="weatherinfo">' +
            strIcon +
            '<span class="temperature">' +
            Math.round(thisWeather.main.temp) +
            "&deg;C</span>" +
            thisWeather.weather[0].main +
            "</p>";
      ...
      if (!boolWeatherFound) {
        strResult +=
          "<p>No forecast for this scheduled booking yet. Check back later, when the booking is within 5 days.</p>";
      }
    }
    ...
    if (!strResult) {
      ...
      strResult =
        "<h1>Scheduled Bookings for Job #" +
        objJob.generated_job_id +
        "</h1>" +
        strResult +
